...
from src.gillespie_models import RESULTS_DIR
from src.manual_revision import DICT_REVISION
from src.settings import VAR_NAME_DICT, COLOURS, IMSHOW_KW, NPZ_SHORT_FILE, VAR_SYM_DICT, MANU_FIG_DIR

np.seterr(divide='ignore', invalid='ignore')


def figure_richness_phases(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, pbx=20, pby=20):
    """
    ...
    """
    POINTS_BETWEEN_X_TICKS = pbx
    POINTS_BETWEEN_Y_TICKS = pby
    ...
    plt.imshow(sim_rich_cat.T, cmap=cmap, norm=norm, aspect='auto')

    # contours
    mf_rich_cat = scipy.ndimage.filters.gaussian_filter(mf_rich_cat, 1.0)  # smooth

    ax.contour(mf_rich_cat.T, [-0.5], linestyles='solid', colors='k', linewidths=2)
    ax.contour(mf_rich_cat.T, [0.5], linestyles='solid', colors='k', linewidths=2)

    pltfcn.set_axis(ax, plt, POINTS_BETWEEN_X_TICKS, POINTS_BETWEEN_Y_TICKS, rangex, rangey, xlog, ylog, xdatalim, ydatalim, xlabel, ylabel)
    ...
    plt.plot(data['rich_dist'][0, R], color=line_colours[0], marker='s', markeredgecolor='None', linewidth=2, zorder=10)
    plt.plot(data['rich_dist'][int(5 * (R) / 10), int(8 * (R) / 10)], color=line_colours[1], marker='o', markeredgecolor='None', linewidth=2, zorder=10)
    plt.plot(data['rich_dist'][R, 0], color=line_colours[2], marker='D', markeredgecolor='None', linewidth=2, zorder=10)
    plt.ylim([0, 1.0])
...
def figure_modality_phases(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, dstbnPlots=False, pbx=20, pby=20):
    POINTS_BETWEEN_X_TICKS = pbx
    POINTS_BETWEEN_Y_TICKS = pby
    """
    Heatmap of modality
    ...
    rangex = data[xlabel][start:]
    rangey = data[ylabel][:]
    K = data['carry_capacity']
    rminus = data['death_rate']
    ...
    approximations =\
        {r'$\langle J | n \rangle = (S-1) \langle n \rangle + n$': mf_dist, r'$\langle J | n \rangle = S \langle n \rangle$': mf3_dist, r'$\langle n_i | n_j \rangle = n_{det}$': mfdet_dist}
    ...
    if xlabel == 'immi_rate':
        mu = (rangex * np.ones((np.shape(sim_dist)[0], np.shape(sim_dist)[1])).T).T

    # categorizing modality for simulation
    modality_sim, line_names, line_colours = pltfcn.determine_modality(
            sim_dist, dstbnPlots, revision, False, data['nbr_species'], approximations)
    ...
    plt.imshow(modality_sim.T, cmap=cmap, norm=norm, aspect='auto')
    ...
    BI = ax.contour(boundaryI.T, [1.0], linestyles='solid', colors='k', linewidths=2)
    BIII = ax.contour(boundaryReIII.T, [1.0], linestyles='solid', colors='b', linewidths=2)
    ...
    h1, _ = BI.legend_elements()
    h2, _ = BIII.legend_elements()
    h = [h1[0], h2[0]]
...
def figure_regimes(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, distplots=False, pbx=20, pby=20):
    """
    Heatmap of modality
    ...
    sim_rich_cat, mf_rich_cat, lines_rich, mf_rich, sim_rich =\
        figure_richness_phases(filename, False, xlabel, ylabel, xlog, ylog, ydatalim, xdatalim, pbx, pby)
    modality_sim, hubbelRegime, mf_unimodal, lines_mod, colours_mod =\
        figure_modality_phases(filename, False, xlabel, ylabel, xlog, ylog, ydatalim, xdatalim, revision, distplots, pbx, pby)
    ...
    plt.imshow(combined_sim.T, cmap=cmap, norm=norm, aspect='auto')
    ...
    #                    , colors = 'k', linewidths = 2)
    ax.contour(hubbelRegime.T, [1.], linestyles='solid', colors='k', linewidths=2)
    ax.contour(mf_unimodal.T, [1.], linestyles='solid', colors='k', linewidths=2)
    if start == 0:
...
def mfpt(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, distplots=False, pbx=20, pby=20):
    """
    ...
    # 2D rho-mu
    mu = (rangex * np.ones((np.shape(data['sim_dist'])[0], np.shape(data['sim_dist'])[1])).T).T

    rho = (rangey * np.ones((np.shape(data['sim_dist'])[0], np.shape(data['sim_dist'])[1])))
    ...
    for i in np.arange(np.shape(arr)[0]):
        for j in np.arange(np.shape(arr)[1]):
            nbr_species_arr[i, j] = S * (1.0 - arr[i, j, 0])
            nbr = int(eq.deterministic_mean(nbr_species_arr[i, j], mu[i, j], rho[i, j], rplus, rminus, K))
            ntilde[i, j] = nbr
            # max_arr[i,j] = nbr
            ...
            dom_turnover[i, j] = eq.mfpt_a2a(arr[i, j], nbr, mu[i, j], rplus, rminus, K, rho[i, j], S)
    ...
    weighted_timescale = ((S - nbr_species_arr) * sub_turnover + nbr_species_arr * dom_turnover) / S

    color_bad = (211 / 256, 211 / 256, 211 / 256)

    sim_rich_cat, mf_rich_cat, lines_rich, _, _ =\
        figure_richness_phases(filename, False, xlabel, ylabel, xlog, ylog, ydatalim, xdatalim, pbx, pby)
    mf_rich_cat = scipy.ndimage.filters.gaussian_filter(mf_rich_cat, 0.7)
    ...
    imshow_kw = {'cmap': my_cmap, 'aspect': None, 'interpolation': None, 'norm': mpl.colors.LogNorm()}
    # heatmap
    ...
    plt.contour(modality_mf.T, [0.5], colors='b')
    plt.clabel(contours, inline=True, fontsize=8, fmt=fmt)
    plt.imshow(ratio_dominance_loss.T, **imshow_kw)

    # ax.clabel(ax1, inline=True, fmt=ticker.LogFormatterMathtext(), fontsize=font_label_contour)
    pltfcn.set_axis(ax, plt, pbx, pby, rangex, rangey, xlog, ylog, xdatalim, ydatalim, xlabel, ylabel)
    # colorbar
    ...
    for i, val in enumerate(plots):
        ...
        plt.plot(rangey[ydatalim[0]:ydatalim[1] + 1],
                 array[ydatalim[0]:ydatalim[1] + 1],
                 markerline[i], c=colours_line[i],
                 label=VAR_SYM_DICT[xlabel] + ": " + r'$10^{%d}$' % np.log10(round(rangex[val], 13)))
    ...
    _ = eq.mfpt_return_full(fNmin, mu, nbr_species_arr, S)

    contours = plt.contour(ratio_suppression_loss.T, [1E-2, 1E-1, 1E0], linestyles='dashed', colors='k')
    plt.contour(mf_rich_cat.T, [0.5], colors='k')
    plt.clabel(contours, inline=True, fontsize=8, fmt=fmt)
    plt.imshow(ratio_suppression_loss.T, **imshow_kw)
    ...
    for i, val in enumerate(plots):
        ...
        array[array == np.inf] = np.nan
        plt.plot(rangey[ydatalim[0]:ydatalim[1]+1], array[ydatalim[0]:ydatalim[1]+1], markerline[i], c=colours_line[i], label=VAR_SYM_DICT[xlabel] + ": " + r'$10^{%d}$' %
...
def supp_mfpt(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, distplots=False, pbx=20, pby=20):
    ...
    imshow_kw = {'cmap': my_cmap, 'aspect': None, 'interpolation': None, 'norm': mpl.colors.LogNorm()}
    # heatmap
    contours = plt.contour(ratio_turnover.T, [1E0, 1E1, 1E2], linestyles='dashed', colors='k')
    plt.clabel(contours, inline=True, fontsize=8, fmt=fmt)
    # contours = plt.contour(ratio_turnover.T, [S-1.], linestyles='dashed', colors='k')
    # plt.clabel(contours, inline=True, fontsize=8, fmt='S-1')
    plt.contour(modality_mf.T, [0.5], colors='b')
    ...
    imshow_kw = {'cmap': my_cmap, 'aspect': None, 'interpolation': None, 'norm': mpl.colors.LogNorm()}
    # heatmap
    # contours = plt.contour(ratio_switch.T, [1E-3, 1E-2, 1E-1], linestyles='dashed', colors='k')
    # plt.clabel(contours, inline=True, fontsize=8, fmt=fmt)
    # contours = plt.contour(ratio_switch.T, [1. / (S - 1.)], linestyles='dashed', colors='k')
    # plt.clabel(contours, inline=True, fontsize=8, fmt='1/(S-1)')
    plt.contour(mf_rich_cat.T, [0.5], colors='k')
...
def many_parameters_dist(filename, save=False, range='comp_overlap', fixed=2, start=0, xlabel='immi_rate', ylabel='comp_overlap'):

    data = np.load(filename)
    plt.style.use('src/custom_heatmap.mplstyle')
    rangex = data[xlabel]
    rangey = data[ylabel][start:]
    if range == xlabel:
        other = rangey
        otherLabel = ylabel
        plotRange = rangex
    ...
    divide = 2
    plotRange = plotRange[::divide]  # divide reduces number points

    for j, element in enumerate(plotRange):
        color = cmap(j / len(plotRange))
        if range == xlabel:
            plt.plot(np.arange(0, np.shape(data['sim_dist'])[2]), data['sim_dist'][divide * j][start + fixed], lw=2, c=color)
        else:
            plt.plot(np.arange(0, np.shape(data['sim_dist'])[2]), data['sim_dist'][fixed][start + divide * j], lw=2, c=color)
    ...
    sm = plt.cm.ScalarMappable(cmap=cmap,
                               norm=mpl.colors.LogNorm(vmin=np.min(plotRange),
                                                       vmax=np.max(plotRange)))
    clb = plt.colorbar(sm)
...
def compare_assymptotics(filename, save=False, range='immi_rate', fixed=40, start=0, xlabel='immi_rate', ylabel='comp_overlap'):
    ...
    titles = [r'simulation', r'$\langle J |n \rangle \approx n$', r'Baxter, Blythe \& McKane', r'Alonso, McKane \& Cole']
    maxn = 500  # 10**4
    ax = [0, 0, 0, 0]
    ...
    for j, element in enumerate(plotRange):
        nOA, ourAppr = theqs.our_approx_neutral(data, immi_rate=element, comp_overlap=1.0, maxn=maxn)
        nBA, baxAppr = theqs.baxter(data, immi_rate=element, maxn=maxn)
        nAA, aloAppr = theqs.alonso(data, immi_rate=element, maxn=maxn)
        # ourAppr = theqs.our_asymp(data, immi_rate=element, comp_overlap=1.0,
        #                          maxn=maxn)
        color = cmap(j / len(plotRange))
        if range == xlabel:
            ax[0].plot(np.arange(0, np.shape(data['sim_dist'])[2]), data['sim_dist'][divide * j][start + fixed], lw=2, c=color)
            ax[1].plot(nOA, ourAppr, lw=2, c=color)
            ax[2].plot(nBA, baxAppr, lw=2, c=color)
            ax[3].plot(nAA, aloAppr, lw=2, c=color)
        else:
            ax[0].plot(np.arange(0, np.shape(data['sim_dist'])[2]), data['sim_dist'][fixed][start + divide * j], lw=2, c=color)
            ax[1].plot(np.arange(maxn), ourAppr, lw=2, c=color)

    for i, axes in enumerate(ax):
        axes.set_yscale('log')
        axes.set_xscale('log')
        # plt.xlim(0,int(1.5*data['carry_capacity']))
        axes.set_ylim(10**(-10), 1.0)
        axes.set_xlabel(r'$n$', fontsize=size, labelpad=2)
        axes.set_ylabel(r'$P(n)$', fontsize=size, labelpad=2)
        axes.set_title(titles[i])
        axes.tick_params(axis='both', which='major', labelsize=size)

    axcbar = fig.add_subplot(gs[:, 2])
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=mpl.colors.LogNorm(
        vmin=np.min(plotRange), vmax=np.max(plotRange)))
    clb = plt.colorbar(sm, cax=axcbar)
    # clb.set_label(VAR_SYM_DICT[range], labelpad=0, y=0.5, x=0.0, rotation=0)
    axcbar.set_title(VAR_SYM_DICT[range])
    # plt.tight_layout()
    fname = 'asymptotic_neutral'
    if save:
        fig.savefig(MANU_FIG_DIR + os.sep + fname + '.pdf')
        fig.savefig(MANU_FIG_DIR + os.sep + fname + '.png')
    else:
        plt.show()
...
def maximum_plot(filename, save=False, range='comp_overlap', start=0, xlabel='immi_rate', ylabel='comp_overlap'):
    ...
    for j, element in enumerate(plotRange):
        ...
        for i, otherVal in enumerate(other):
            ...
            if dstbn[0] > dstbn[1]:
                max0.append(0)
                loc_max0.append(otherVal)
                max_excl0 = np.argmax(dstbn[1:])
            # Max at N
            if max_excl0 != 0:
                maxN.append(max_excl0 + 1)
                loc_maxN.append(otherVal)

        plt.scatter(loc_max0, max0, s=10 / np.sqrt(j + 1), lw=2, c=color, edgecolor='none')
        plt.scatter(loc_maxN, maxN, s=10 / np.sqrt(j + 1), lw=2, c=color, edgecolor='None')

    plt.xscale('log')
    plt.xlim(np.min(other), np.max(other))
    ...
    clb = plt.colorbar(sm)
    clb.set_label(VAR_SYM_DICT[range], labelpad=-30, y=1.1, rotation=0)
    plt.xlabel(VAR_NAME_DICT[otherLabel])
    plt.ylabel(r'$\tilde{n}$')
    if save:
...
def fig_corr(filename, save=False, xlabel='immi_rate', ylabel='comp_overlap', xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, distplots=False, pbx=20, pby=20):
    ...
    correff = ['corr_ni_nj', 'corr_J_n', 'corr_Jminusn_n', 'coeff_ni_nj', 'coeff_J_n', 'coeff_Jminusn_n'
               ]
    corref_name = [r'cov($n_i,n_j$)/$\sigma_{n_i}\sigma_{n_j}$', r'cov($J,n$)/$\sigma_{J}\sigma_{n}$', r'cov($J-n,n$)/$\sigma_{J-n}\sigma_{n}$', r'cov($n_i,n_j$)/$\langle n_i \rangle \langle n_j\rangle$', r'cov($J,n$)/$\langle J \rangle \langle n \rangle$', r'cov($J-n,n$)/$\langle (J-n) \rangle \langle n \rangle$'
                   ]
    ...
    plt.gcf()
    ax = plt.gca()
    plt.imshow((data[score]).T, **imshow_kw)
    pltfcn.set_axis(ax, plt, pbx, pby, rangex, rangey, xlog, ylog, xdatalim, ydatalim, xlabel, ylabel)
...
def fig_timecorr(sim, sim_nbr, save=False, xlabel='immi_rate', ylabel='comp_overlap', start=0, xlog=True, ylog=True, ydatalim=None, xdatalim=None, revision=None, pbx=20, pby=20):
    ...
    autocor, spectrum, specAutocor, specSpectrum, newTimes = autospec(data['results']['times'][n:],
...
def fig_timescales_autocor(sim,
                           save=False,
                           range='comp_overlap',
                           start=0,
                           xlabel='immi_rate',
    ...
    for j, element in enumerate(plotRange):
        color = cmap(j / len(plotRange))
        ax.plot(other, data['time_autocor_spec'][j, :], lw=2, fmt='-o', c=color)
        plt.scatter(other, data['suppress_return'][j, :] + ['dominance_turnover'][j, :], lw=2, c=color, edgecolor='none')
    ax.plot(np.NaN, np.NaN, yerr=np.NaN, fmt='-o', color='silver', label=r'$T_{exp}$ : $e^{-t/T_{exp}}$')
    plt.scatter(np.NaN, np.Nan, lw=2, c='silver', edgecolor='none', label=r'$\langle T_0(\tilde{n}) \rangle + \langle T(0\rightarrow \tilde{n}) \rangle$')
    ...
    plt.ylabel(r'times')
    if save:
        plt.savefig(MANU_FIG_DIR + os.sep + range + '_' + 'species_timescale_cor' + '.pdf')
        # plt.savefig(MANU_FIG_DIR + os.sep + range + '_' + 'maximums' +'.png');
        plt.close()
    else:
        plt.show()
    ...
    for j, element in enumerate(plotRange):
        color = cmap(j / len(plotRange))
        ax.errorbar(other, data['mean_time_autocor_abund'][j, :], yerr=data['std_time_autocor_abund'][j, :], fmt='-o', c=color)
        plt.scatter(other, data['suppress_turnover'][j, :], lw=2, c=color, edgecolor='none', marker='o')
        plt.scatter(other, data['dominance_turnover'][j, :], lw=2, c=color, edgecolor='none', marker='x')
    ax.errorbar(np.NaN, np.NaN, yerr=np.NaN, fmt='-o', color='silver', label=r'$T_{exp}$ : $e^{-t/T_{exp}}$')
    plt.scatter(np.NaN, np.Nan, lw=2, c='silver', edgecolor='none', marker='o', label=r'$\langle T(0\rightarrow 0) \rangle$')
    plt.scatter(np.NaN, np.Nan, lw=2, c='silver', edgecolor='none', marker='x', label=r'$\langle T(\tilde{n}\rightarrow \tilde{n}) \rangle$')
    ...
    if save:
        plt.savefig(MANU_FIG_DIR + os.sep + range + '_' + 'abundance_timescale_cor' + '.pdf')
        # plt.savefig(MANU_FIG_DIR + os.sep + range + '_' + 'maximums' +'.png');
        plt.close()
    else:
        plt.show()
...
def plot_trajectory(dir, sim_nbr, nbr_steps, plot_dstbn=True, colour='red', save=False):
    """
    Plot species trajectories for the t=time last reactions. May also print the
    distribution on the side if needed.
    """
    with open(dir + os.sep + 'sim' + str(sim_nbr) + os.sep + 'results_0.pickle', 'rb') as handle:
        data = pickle.load(handle)
    ...
    if plot_dstbn:
        fig, (ax1, ax2) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [3, 1]}, figsize=(4, 2.5))
    else:
        fig, ax1 = plt.figure(figsize=(3.25, 2.5))
    ...
    for i in np.arange(np.shape(trajectories)[1]):
        ...
        ax1.set_xlabel('time')
        ax1.set_ylabel(r'abundance, $n$')
        ax1.set_xlim(left=min(times), right=max(times))

    if plot_dstbn:
        ax2.plot(ss_dist_sim, np.arange(0, len(ss_dist_sim)), color=colour, linewidth=3)
        ax2.set_xscale('log')
...
if __name__ == "__main__":
    ...
    # compare_assymptotics(sim_K100 + os.sep + NPZ_SHORT_FILE, save=True, fixed=40)
