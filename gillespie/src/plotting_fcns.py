...
def plot_prob(probability, i, j, colour, SRA=False, approx_dstbn={}, nbrSpecies=30):
    ...
    scalefig = 2
    if not SRA:
        fsize = (scalefig * 1.75, scalefig * 1.25)
        cols = 1
    else:
        fsize = (scalefig * 3.5, scalefig * 1.25)
        cols = 2
    fig = plt.figure(figsize=fsize)
    gs = fig.add_gridspec(1, cols)
    ax = fig.add_subplot(gs[0, 0])
    ax.plot(probability[:maxn], color=colour, linewidth=lw, zorder=10, label='sim.')
    for k, key in enumerate(approx_dstbn):
        ax.plot(approx_dstbn[key][:maxn], color='k', linewidth=lw, zorder=10, linestyle=lstyle[k], label=key)
    ...
    if SRA:
        ...
        ax2.plot(rank, SRA, color=colour, linewidth=lw, zorder=10)
        for k, key in enumerate(approx_dstbn):
            rank, SRA = SAD_to_SRA(np.arange(0, maxn), nbrSpecies * approx_dstbn[key][:maxn])
            ax2.plot(rank, SRA, color='k', linewidth=lw, zorder=10, linestyle=lstyle[k], label=key)
        ax2.axes.labelsize = size
        ax2.set_xlabel(r'rank', fontsize=size, labelpad=2)
        ax2.set_ylabel(r'abundance', fontsize=size, labelpad=2)
        ax2.set_xlim([0, nbrSpecies])
...
def SAD_to_SRA(n, SAD):
    rank = np.sum(SAD) - np.cumsum(SAD)
    SRA = n
    return rank, SRA


def determine_modality(arr, plot=True, revisionmanual=None, sampleP0=True, nbrSpecies=30, approx_dict={}, SRA=True):
    """
    ...
    line_colours = ['khaki', 'slateblue', 'mediumturquoise', 'lightcoral', 'lightgray']
    line_names = ['unimodal\n peak at 0', 'unimodal\n peak at >0', 'bimodal', 'multimodal', r'$P(0)$ unsampled']
    ...
    if plot:
        for i in np.arange(np.shape(arr)[0]):
            for j in np.arange(np.shape(arr)[1]):
                ...
                plot_prob(arr[i, j, :], i, j, line_colours[int(modality_arr[i, j])], SRA, approx_dict_ij)
...
def heatmap(xrange, yrange, arr, xlabel, ylabel, title, dir, pbtx=10, pbty=20, save=False, xlog=True, ylog=True, xdatalim=None, ydatalim=None):
    # TODO : CHANGE AXES
    ...
    # labels and ticks
    set_axis(ax, plt, pbtx, pbty, xrange, yrange, xlog, ylog, xdatalim, ydatalim, xlabel, ylabel)
...
def set_axis(ax, plt, POINTS_BETWEEN_X_TICKS, POINTS_BETWEEN_Y_TICKS, rangex, rangey, xlog, ylog, xdatalim, ydatalim, xlabel, ylabel):
    ...
    if ylog:
        ...
        ax.set_yticklabels([r'$%.2f$' % round(yval, 2)
                            ...
                            if i % POINTS_BETWEEN_Y_TICKS == 0])
    ax.invert_yaxis()
    if ydatalim is not None:
        plt.ylim([ydatalim[0], ydatalim[1]])
    if xdatalim is not None:
        plt.xlim([xdatalim[0], xdatalim[1]])

    plt.xlabel(VAR_NAME_DICT[xlabel])
    plt.ylabel(VAR_NAME_DICT[ylabel])
